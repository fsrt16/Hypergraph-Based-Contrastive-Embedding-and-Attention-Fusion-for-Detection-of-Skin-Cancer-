...
class GraphTraining:
    ...
    def train(self):
        # Training loop
        # compute tiem for training and testing
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            with torch.no_grad():
                ...
                validation_time = end - start_time
            ...
            print(f"Epoch [{epoch}/{self.num_epochs}] Time {training_time}| Train Loss: {avg_train_loss:.4f}  | Time {validation_time} | Val Loss: {avg_val_loss:.4f}")
            ...
            if avg_val_loss < self.best_val_loss:
                ...
                print(f"-------------- Validation Loss Imporved from {self.best_val_loss:.4f} to {avg_val_loss:.4f}")

                torch.save(self.model.state_dict(), os.path.join(self.save_path, "CHCK__best_model.pt"))
        ...
        # Save final model
        torch.save(self.model.state_dict(), os.path.join(self.save_path, "CHCK__final_model.pt"))
...
class Visualizer:
    ...
    def _plot_embeddings(self, embeddings_list, labels_list, titles, axis_labels, title):
        ...
        legend_handles = [Patch(color=color, label=label) for label, color in self.color_mapping.items()]

        # Adjust figure size and layout for legend outside the plot
        fig.subplots_adjust(right=0.8)  # Make space on the right

        # Put legend outside the plot area (right side), stacked vertically
        fig.legend(
            handles=legend_handles,
            loc='center left',
            bbox_to_anchor=(1.02, 0.5),  # Place legend to the right of the figure
            borderaxespad=0.0,
            fontsize=12,
            frameon=False
        )
        plt.tight_layout(rect=[0, 0, 0.85, 1])  # Leave space for the external legend
